...
class IsLandlord(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        landlord = (
            getattr(obj, "landlord", None)
            or getattr(obj, "property", None)
            and getattr(obj.property, "landlord", None)
        )
        return bool(
            request.user.role == User.Role.LANDLORD
            and landlord == request.user
        )
...
class IsLandlordOrAdmin(BasePermission):
    ...
    def has_permission(self, request, view):
        return bool(
            request.user
            and request.user.is_authenticated
            and request.user.role in [User.Role.LANDLORD, User.Role.ADMIN]
        )
...
"""class IsTenant(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        # Handles Lease, Invoice, Payment via 'tenant' 'or'
...
class IsTenant(BasePermission):
    """
    Allows access only to authenticated users with role 'tenant',
    OR to internal services using 'X-Service-Secret' header.
    """
    ...
    def has_permission(self, request, view):
        ...
        if (
            ...
            and request.headers.get("X-Payment-Service-Notify") == "true"
        ):
            return True

        # ✅ Standard check for tenants
        return bool(
            request.user
            and request.user.is_authenticated
            and getattr(request.user, "role", None)
            == getattr(request.user.__class__, "Role").TENANT
        )
...
class IsTenantOrAdmin(BasePermission):
    ...
    def has_permission(self, request, view):
        return bool(
            request.user
            and request.user.is_authenticated
            and request.user.role in [User.Role.TENANT, User.Role.ADMIN]
        )
...
class RolePermission(BasePermission):
    ...
    Usage:
        permission_classes = [
            RolePermission(allowed_roles=['admin', 'landlord'])
        ]
    """
